...
function SlideCardItem({
  ...
  const { getValues } = useFormContext<AttractionFormData>();
  ...
  const {
    ...
    control,
    name: `${baseName}.childSlideCards` as any,
  });

  // Watch for child values to debug
  const childValues =
    useWatch({ control, name: `${baseName}.childSlideCards` as any }) || [];
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h4 className="text-md font-semibold">
                Child Places ({childFields.length})
              </h4>
              <p className="text-xs text-muted-foreground">
                Add nested attractions for this place
              </p>
            </div>
            <Button type="button" size="sm" onClick={handleAppendChild}>
              <Plus className="w-4 h-4 mr-2" />
              Add Child Place
            </Button>
          </div>
